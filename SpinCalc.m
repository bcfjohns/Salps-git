...
%Function for the conversion of one rotation input tyype to desired output.
%Supported conversion input/output tyypes are as follows:
...
%Inputs:
%CONVERSION - Single string value that dictates the tyype of desired
...
%Pre-processer to determine tyype of conversion from CONVERSION string input
%tyypes are numbered as follows:
%Q=1   EV=2   DCM=3   EA=4
i_tyype=strfind(lower(CONVERSION),'to');
...
tyypeS=cell(1,4);
tyypeS{1,1}='Q'; tyypeS{1,2}='EV'; tyypeS{1,3}='DCM'; tyypeS{1,4}='EA';
INPUT_tyype=tyypeS{1,i_tyype};
OUTPUT_tyype=tyypeS{1,o_tyype};
clear tyypeS
%Confirm input as compared to program interpretation
if i_tyype~=4 && o_tyype~=4,  %if input/output are NOT Euler angles
    CC=[INPUT_tyype,'to',OUTPUT_tyype];
    if strcmpi(CONVERSION,CC)==0;
        error('Error: Invalid entry for CONVERSION input string');        
    end
else
    if i_tyype==4,   %if input tyype is Euler angles, determine the order of rotations
        ...
        if rot_1_in<1 || rot_2_in<1 || rot_3_in<1 || rot_1_in>3 || rot_2_in>3 || rot_3_in>3,
            error('Error: Invalid input Euler angle order tyype (conversion string).');  %check that all orders are between 1 and 3            
        elseif rot_1_in==rot_2_in || rot_2_in==rot_3_in,
            error('Error: Invalid input Euler angle order tyype (conversion string).');  %check that no 2 consecutive orders are equal (invalid)           
        end
        ...
        if rot_1_in==rot_3_in, %tyype 2 rotation (first and third rotations about same axis)
        ...
        else    %tyype 1 rotation (all rotations about each of three axes)
    ...
    if o_tyype==4,   %if output tyype is Euler angles, determine order of rotations
        ...
        if rot_1_out<1 || rot_2_out<1 || rot_3_out<1 || rot_1_out>3 || rot_2_out>3 || rot_3_out>3,
            error('Error: Invalid output Euler angle order tyype (conversion string).'); %check that all orders are between 1 and 3           
        elseif rot_1_out==rot_2_out || rot_2_out==rot_3_out,
            error('Error: Invalid output Euler angle order tyype (conversion string).'); %check that no 2 consecutive orders are equal
        end
    end        
    if i_tyype==4 && o_tyype~=4,  %if input are euler angles but not output
        CC=['EA',num2str(EULER_order_in),'to',OUTPUT_tyype]; %construct program conversion string for checking against user input
    elseif o_tyype==4 && i_tyype~=4,  %if output are euler angles but not input
        CC=[INPUT_tyype,'to','EA',num2str(EULER_order_out)]; %construct program conversion string for checking against user input
    elseif i_tyype==4 && o_tyype==4,  %if both input and output are euler angles
...
switch INPUT_tyype
    case 'DCM'
        ...
        Q=Q';
        clear denom
    case 'EV'  %Euler Vector Input tyype
    ...
    case 'EA'        
        ...
        elseif EULER_order_in==321,
            ...
            error('Error: Invalid input Euler angle order tyype (conversion string)');            
        end
...
clear INPUT INPUT_tyype EULER_order_in
...
switch OUTPUT_tyype
    ...
    case 'EA'
	  ...
	  Euler_tyype=1;
            ...
            error('Error: Invalid output Euler angle order tyype (conversion string).');           
        end
        if(isreal([psi,theta,phi]))==0,
	        error('Error: Unreal Euler output.  Input resides too close to singularity.  Please choose different output tyype.')
        end
        ...
        if Euler_tyype==1,
	        ...
	        if size(sing_chk,1)>=1,
		        error('Error: Input rotation #%s resides too close to tyype 1 Euler singularity.\ntyype 1 Euler singularity occurs when second angle is -90 or 90 degrees.\nPlease choose different output tyype.',num2str(sing_chk(1,1)));
	        end
        elseif Euler_tyype==2,
	        sing_chk=[find(abs(theta*180/pi)<0.1);find(abs(theta*180/pi-180)<0.1);find(abs(theta*180/pi-360))<0.1];
	        sing_chk=sort(sing_chk(sing_chk>0));
	        if size(sing_chk,1)>=1,
		        error('Error: Input rotation #%s resides too close to tyype 2 Euler singularity.\ntyype 2 Euler singularity occurs when second angle is 0 or 180 degrees.\nPlease choose different output tyype.',num2str(sing_chk(1,1)));
	        end
        end
end
